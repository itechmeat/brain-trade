...
import { executeAIAnalysisWithRetry, executeDocumentGenerationWithRetry } from './gemini-utils';
import {
...
/**
 * Generate raw text response with OpenRouter (for document generation)
 */
async function generateWithOpenRouterRaw(
  prompt: string,
...
/**
 * Generate document from business idea using AI
 */
export async function generateDocumentFromIdea(
  prompt: string,
  selectedModel?: AvailableModel,
): Promise<{ result: DocumentGenerationResult; attempts: number; model: string }> {
  log('[Document Generation] Starting document generation...', {
    promptLength: prompt.length,
    selectedModel: selectedModel || 'default',
  });
  ...
  log('[Document Generation] Using provider config:', {
    provider: providerConfig.provider,
    model: providerConfig.model,
  });

  let generationResult: { result: DocumentGenerationResult; attempts: number };

  try {
    switch (providerConfig.provider) {
      case 'gemini':
        log('[Document Generation] Delegating to Gemini...');
        generationResult = await executeDocumentGenerationWithRetry(async () => {
          return await generateWithGeminiRaw(prompt, providerConfig.apiKey);
        });
        break;
      case 'openrouter':
        log('[Document Generation] Delegating to OpenRouter...');
        const openRouterResult = await generateWithOpenRouter(prompt, {
          apiKey: providerConfig.apiKey,
          model: providerConfig.model,
        });

        // Parse and validate the result for document generation
        ...
        try {
          ...
          throw new APIError('Failed to parse OpenRouter response as JSON', 500);
        }

        const validatedResult = DocumentGenerationResultSchema.parse(parsedResult);
        generationResult = { result: validatedResult, attempts: openRouterResult.attempts };
        break;
      default:
        throw new APIError(`Unsupported AI provider: ${providerConfig.provider}`, 500);
    }

    log('[Document Generation] Generation completed successfully:', {
      provider: providerConfig.provider,
      model: providerConfig.model,
    ...
    logError('[Document Generation] Generation failed with detailed context:', errorDetails);
    ...
    if (
      ...
      logError(
        '[Document Generation] JSON/Parsing error detected - this might be due to AI model returning invalid format:',
        {
          errorType: 'JSON_PARSING_ERROR',
          model: providerConfig.model,
          provider: providerConfig.provider,
      ...
      throw new APIError(
        `AI model ${providerConfig.model} returned invalid JSON format for document generation: ${error.message}. This model may need different prompting strategy.`,
        500,
      );
    }
...
/**
 * Generate document update based on recommendations
 * @param currentDocument - Current project document
...
export async function generateDocumentUpdate(
  ...
  log('[Document Update] Starting document update generation...');

  const providerConfig = getAIProviderConfig(selectedModel);

  try {
    // Import prompts dynamically
    const { getDocumentUpdatePrompt, getDocumentUpdateMessagePrompt } = await import(
      '@/lib/prompts'
    );

    // Generate updated document
    const documentPrompt = getDocumentUpdatePrompt({
      currentDocument,
      recommendations: updateRequest,
    });

    log('[Document Update] Generating updated document...', {
      currentDocumentLength: currentDocument.length,
      updateRequestLength: updateRequest.length,
      promptLength: documentPrompt.length,
    });

    let documentResult: { result: string; attempts: number };

    switch (providerConfig.provider) {
      case 'gemini':
        const geminiText = await generateWithGeminiRaw(documentPrompt, providerConfig.apiKey);
        documentResult = { result: geminiText, attempts: 1 };
        break;
      case 'openrouter':
        const openRouterText = await generateWithOpenRouterRaw(documentPrompt, {
          apiKey: providerConfig.apiKey,
          model: providerConfig.model,
        });
        documentResult = { result: openRouterText, attempts: 1 };
        break;
      default:
        throw new APIError(`Unsupported AI provider: ${providerConfig.provider}`, 500);
    }
    ...
    const messagePrompt = getDocumentUpdateMessagePrompt({
      chatHistory,
      changes: updateRequest,
    });

    log('[Document Update] Generating assistant message...');

    let messageResult: {
      result: { assistantMessage?: string; [key: string]: unknown };
      attempts: number;
    };

    switch (providerConfig.provider) {
      case 'gemini':
        messageResult = (await generateWithGemini(
          messagePrompt,
        ...
        break;
      case 'openrouter':
        messageResult = (await generateWithOpenRouter(messagePrompt, {
          apiKey: providerConfig.apiKey,
          model: providerConfig.model,
        })) as unknown as {
          result: { assistantMessage?: string; [key: string]: unknown };
    ...
    const assistantMessage =
      messageResult.result.assistantMessage || 'Document updated with your requested changes.';

    log('[Document Update] Document update completed successfully:', {
      updatedDocumentLength: documentResult.result.length,
      assistantMessageLength: assistantMessage.length,
      documentAttempts: documentResult.attempts,
      messageAttempts: messageResult.attempts,
    });

    return {
      updatedDocument: documentResult.result,
      assistantMessage,
    };
  } catch (error) {
    const errorDetails = {
    ...
    logError(
      '[Document Update] Document update generation failed with detailed context:',
      errorDetails,
    );
