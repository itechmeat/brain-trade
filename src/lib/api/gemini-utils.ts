...
/**
 * Parse and validate AI response for document generation
 */
export function parseAndValidateDocumentResponse(rawResponse: string): DocumentGenerationResult {
  console.log('Parsing document generation response:', {
  ...
  try {
    ...
    try {
      ...
      console.error('Cleanup parse failed:', {
        error: cleanupError instanceof Error ? cleanupError.message : String(cleanupError),
      });
      
      // If JSON parsing fails completely, we'll throw an error
      throw new Error(`Failed to parse AI response as JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  console.log('Successfully parsed JSON, validating schema...');

  // Validate the parsed result
  try {
    const validatedResult = DocumentGenerationResultSchema.parse(parsed);
    console.log('Document generation response validated successfully');
    return validatedResult;
  } catch (validationError) {
    console.error('Document generation validation failed:', validationError);
    ...
    console.error('Document generation validation failed:', {
      errors,
      parsedObject: parsed,
    });

    throw new Error(`Document generation validation failed: ${errors.map(e => `${e.field}: ${e.message}`).join(', ')}`);
  }
}

/**
 * Execute document generation with retry logic
 */
export async function executeDocumentGenerationWithRetry(
  apiCall: () => Promise<string>,
  config: RetryConfig = DEFAULT_RETRY_CONFIG,
): Promise<{ result: DocumentGenerationResult; attempts: number }> {
  const { maxRetries, baseDelay } = config;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Parse and validate the response
      const validatedResult = parseAndValidateDocumentResponse(rawResponse);

      return { result: validatedResult, attempts: attempt + 1 };
    } catch (error) {
      console.error(`Document generation attempt ${attempt + 1} failed:`, error);

      // On the last attempt, throw the error
      if (attempt === maxRetries) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        throw new APIError(
          `Document generation failed after ${attempt + 1} attempts: ${errorMessage}`,
          500,
        );
      }
      ...
      // Log retry attempt
      console.warn(
        `Document generation error on attempt ${attempt + 1}, retrying in ${baseDelay * (attempt + 1)}ms...`,
        error instanceof Error ? error.message : String(error),
      );
  ...
  // This should never be reached, but just in case
  throw new APIError('Document generation failed after all retries', 500);
}
